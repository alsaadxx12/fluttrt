...
/// «قائمة المفضلة» as its own page: the title bar with the count, over
/// [CinemanaFavoritesBody].
class CinemanaFavoritesScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      // Light mode: pure white page, containers set apart by shadows (dark mode unchanged).
      ...
      body: const CinemanaFavoritesBody(),
    );
  }
}

/// Everything below the favourites page's title: the kind filter and the
/// grid of posters (or the empty state). Has no header of its own, so it can
/// sit under the page's bar or inside «قائمتي».
class CinemanaFavoritesBody extends ConsumerStatefulWidget {
  const CinemanaFavoritesBody({super.key});

  @override
  ConsumerState<CinemanaFavoritesBody> createState() => _CinemanaFavoritesBodyState();
}

class _CinemanaFavoritesBodyState extends ConsumerState<CinemanaFavoritesBody> {
              ...
              : GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return CinemanaPosterCard(
                      item: item,
                      onTap: () {
                        // The root navigator: from inside the shell («قائمتي»)
                        // the detail must open over the whole app, not under
                        // the bottom bar.
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected
              ? Colors.redAccent
              : (isDark ? const Color(0xFF1E1E26) : palette.card),
          borderRadius: BorderRadius.circular(20),
          border: isDark ? null : Border.all(color: isSelected ? Colors.redAccent : palette.border),
        ),
        child: Text(
          label,
          style: TextStyle(
