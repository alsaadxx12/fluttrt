...
class SportsService {
  ...
  Future<List<LeagueGroup>> fetch365Matches({String day = 'today'}) async {
    try {
      ...
      final comps = {
        for (var c in (data['competitions'] as List? ?? []))
          if (c is Map && c['id'] != null) c['id']: c
      };
      ...
      for (final g in games) {
        ...
        final hName = hComp['name']?.toString().trim() ?? '';
        final aName = aComp['name']?.toString().trim() ?? '';
        if (hName.isEmpty || aName.isEmpty) continue;

        final teamKey = '${hName.toLowerCase()}_${aName.toLowerCase()}';
        if (!seenTeams.add(teamKey)) continue;

        final hId = hComp['id'];
        final aId = aComp['id'];
        final hLogo = hId != null
            ? 'https://imagecache.365scores.com/image/upload/f_png,w_48,h_48,c_limit,q_auto:eco,dpr_2/v1/competitors/$hId'
            : null;
        final aLogo = aId != null
            ? 'https://imagecache.365scores.com/image/upload/f_png,w_48,h_48,c_limit,q_auto:eco,dpr_2/v1/competitors/$aId'
            : null;
        ...
        final gameTime = (g['gameTime'] as num?)?.toDouble() ?? -1.0;
        ...
        if (day == 'yesterday') {
          ...
          if (statusGroup == 4 || statusText.contains('انتهت') || statusText.contains('نهائي')) {
            ...
            homeScore = (hComp['score'] as num?)?.toInt();
            awayScore = (aComp['score'] as num?)?.toInt();
          } else if (statusText.contains('لم تبدأ')) {
            status = 'scheduled';
            homeScore = null;
            awayScore = null;
          } else if (statusGroup == 2 || statusGroup == 3 || gameTime > 0) {
            status = 'live';
            homeScore = (hComp['score'] as num?)?.toInt();
            awayScore = (aComp['score'] as num?)?.toInt();
          } else {
        ...
        final startTimeStr = g['startTime']?.toString() ?? '';
        final matchId = (g['id'] as num?)?.toInt() ?? (900000 + matchCounter);
        matchCounter++;
        ...
        final item = SportMatchItem(
          ...
          homeScore: homeScore,
          awayScore: awayScore,
          minute: isMatchLive && gameTime > 0 ? gameTime.toInt() : null,
          hasWatch: hasWatch,
          league: rawLeague,
        );

        groupedByLeague.putIfAbsent(rawLeague, () => []).add(item);
      }
  ...
  String cleanTeamNameForMatch(String name) {
    return name
        ...
        .replaceAll('مانشستر سيتي', 'مان سيتي')
        .replaceAll('مانشستر يونايتد', 'مان يونايتد')
        .replaceAll('باريس سان جيرمان', 'باريس')
        .replaceAll('باريس سان جيرمان - سيدات', 'باريس')
        .replaceAll('ريال سوسييداد', 'ريال سوسيداد')
        .replaceAll('ديبورتيفو لاكورنيا', 'لاكورونيا')
        .replaceAll('إنتر ميامي', 'انتر ميامي')
        .replaceAll('سان دييغو', 'سان دييجو')
        .replaceAll('أ', 'ا')
        .replaceAll('إ', 'ا')
        .replaceAll('آ', 'ا')
        .replaceAll('ة', 'ه')
  ...
  bool matchesTeams(String h1, String a1, String h2, String a2) {
    final ch1 = cleanTeamNameForMatch(h1);
    final ch2 = cleanTeamNameForMatch(h2);
    final ca1 = cleanTeamNameForMatch(a1);
    final ca2 = cleanTeamNameForMatch(a2);
    if (ch1.isEmpty || ch2.isEmpty || ca1.isEmpty || ca2.isEmpty) return false;

    bool matchSingle(String t1, String t2) {
  ...
  /// True when the game found is the match asked about: at least one of
  /// the match's teams is named among the game's competitors. Nothing to
  ...
  static bool teamsAgree(String? homeName, String? awayName, String? gameHome, String? gameAway) {
    ...
    for (final a in asked) {
      for (final f in found) {
        if (a == f || a.contains(f) || f.contains(a)) return true;
        final words = a.split(' ').where((w) => w.length > 2);
        if (words.any((w) => f.contains(w))) return true;
      }
    }
    return false;
  }
  ...
  static String _teamKey(String? name) {
    ...
    s = s
        ...
        .replaceAll(RegExp(r'\s+(fc|cf|sc|u\d+)$'), '')
            ...
            for (final c in comps.take(3)) {
              ...
              if (gRes.statusCode == 200 && gRes.data is Map) {
                ...
                if (games != null && games.isNotEmpty) {
                  ...
                  // No game of this competitor is against the other team:
                  // the competitor is the wrong one (a club that shares a
                  // word with a country, say), not the game. Taking its
                  // first game anyway put Senegal's line-up under Iraq v
                  // Oman. Try the next competitor instead.
                  continue;
                }
              }
            }
  ...
  Future<MatchDetailedInfo?> fetchMatchDetails(
    ...
    // The teams the caller asked about, kept apart from the names the
    // game feed gives further down.
    final askedHome = homeName;
    final askedAway = awayName;
    try {
      ...
      //
      // Checked against the teams the match is actually between: a game
      // whose competitors are neither of them is somebody else's match,
      // whatever id led here, and its line-ups are not shown.
      final gameDio = createDio(BaseOptions(
        connectTimeout: const Duration(seconds: 10),
        receiveTimeout: const Duration(seconds: 10),
        headers: {'User-Agent': 'Mozilla/5.0'},
      ...
      if (gameRes.statusCode == 200 && gameRes.data is Map && gameRes.data['game'] is Map) {
        ...
        if (!teamsAgree(
          askedHome,
          askedAway,
          homeCompetitor?['name']?.toString(),
          awayCompetitor?['name']?.toString(),
        )) {
          debugPrint('[sports] 365scores game $resolvedSourceId is '
              '${homeCompetitor?['name']} v ${awayCompetitor?['name']}, '
              'not $askedHome v $askedAway - line-ups dropped');
          return null;
        }
